...
if ($foundVcvars -and -not (Get-Command cl.exe -ErrorAction SilentlyContinue)) {
    ...
    cmd /c "`"$foundVcvars`" >nul 2>&1 && powershell -NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`""
    exit $LASTEXITCODE
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  [!] Intento con VS 2022 falló, reintentando con generador Ninja..." -ForegroundColor Yellow
    $cmakeArgsAlt = @(
        "-B", "build",
        "-G", "Ninja",
        "-DGGML_CUDA=ON",
...
if (Test-Path $exeLocation) {
    ...
    Write-Host " 🎉 COMPILACIÓN COMPLETADA CON ÉXITO (PORTABLE + BORINGSSL HTTPS + BLACKWELL)" -ForegroundColor Green
