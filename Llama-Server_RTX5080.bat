...
if "!VAL_TAVILY!"=="1" (
    if defined TAVILY_API_KEY (
        ...
        echo {"mcpServers":"[{\"id\":\"tavily-mcp\",\"enabled\":true,\"name\":\"Tavily Search\",\"url\":\"https://mcp.tavily.com/mcp/?tavilyApiKey=!TAVILY_API_KEY!\",\"requestTimeoutSeconds\":300,\"useProxy\":true}]"} > "!JSON_FILE!"
...
echo  ║  KV Cache : !VAL_KV! (Blackwell)
echo  ║  IP LAN   : !L_IP!
...
"!BIN_EXE!" -m "!MODEL_PATH!" --flash-attn on --spec-type ngram-map-k -ngl !VAL_NGL! -c !VAL_CTX! -np !VAL_NP! !KV_ARGS! !B_ARGS! !MCP_ARGS! --temp 0.4 --host !L_HOST! --port !REAL_PORT! --jinja
