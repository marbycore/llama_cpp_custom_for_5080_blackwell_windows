...
## 🧠 Estabilización de Razonamiento Acotado en Qwen 3.8 (Presupuesto a 512 Tokens para Chat API)

### 🔍 Causa Raíz e Infinito Thinking Block
Al solicitar tareas complejas de código monolítico (ej. demostraciones WebGL / Three.js 3D), la serie de modelos **Qwen 3.8** (tanto la versión con censura `Qwen3.8-27B` como la variante sin censura `Qwen3.8-27B-Uncensored`) tendían a quedarse atrapadas de forma indefinida dentro del bloque de razonamiento `<think>`. Esto consumía miles de tokens en análisis repetitivo y agotaba la ventana de generación sin llegar a emitir el código HTML/JS final.

### 🛠️ Solución Implementada en `Llama-Server_RTX5080.bat` para API Chat (`/v1/chat/completions`)
Se implementó la inyección automática de las banderas de control de razonamiento de `llama-server` específicas para el formato DeepSeek/Qwen:

```cmd
:: Resolver Reasoning Budget (Exclusivo para la serie Qwen 3.8)
set "REASONING_ARGS="
...
### ⚡ Resultados en Hermes Agent (`/v1/chat/completions`)
* **Tiempo en Fase de Pensamiento:** Reducido a **~9.3 a 12 segundos** (exactamente 512 tokens).
* **Cierre Automático:** El servidor inyecta de forma limpia el cierre `</think>` al llegar al límite de 512 tokens.
* **Velocidad de Generación de Código:** **>50 - 60 tokens/segundo** sostenidos en la RTX 5080.
* **Aislamiento por Modelo:** Aplicado exclusivamente a la serie Qwen 3.8; los modelos como **Qwen 3.6 35B MoE** u otros mantienen su comportamiento original sin intervención.
