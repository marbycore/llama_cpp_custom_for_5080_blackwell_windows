...
* **Repositorio de Trabajo:** `c:\data\llama-cpp-custom` (`marbycore/llama_cpp_custom_for_5080_blackwell_windows`).

---

### 🎛️ Especificación Completa de Lanzamiento: Los 2 Scripts `.bat`

Para garantizar la recreación exacta sin estimaciones ni omisiones, a continuación se detallan los comandos completos y desglosados de los dos archivos de arranque ubicados en `C:\Users\<TU_USUARIO>\Desktop\` y sincronizados en `c:\data\llama-cpp-custom\`:

#### ⚡ 1. Script MTP Dedicado (`Llama-Server_RTX5080_MTP.bat`)
* **Uso:** Exclusivo para modelos que integran cabezas MTP (Multi-Token Prediction) como `Qwen3.6-35B-A3B-MTP`.
* **Ruta de Ejecución:** `build\bin\llama-server.exe`
* **Comando Literal Exacto:**
  ```cmd
  build\bin\llama-server.exe -m "C:\Users\<TU_USUARIO>\.lmstudio\models\unsloth\Qwen3.6-35B-A3B-MTP-IQ3_XXS-GGUF\Qwen3.6-35B-A3B-UD-IQ3_XXS.gguf" --flash-attn on -ngl 99 -c 65536 -np 1 -ctk q4_0 -ctv q4_0 --spec-type draft-mtp --spec-draft-n-max 2 --host 127.0.0.1 --port 5050 --jinja
  ```

#### 🛡️ 2. Script Estándar Fallback (`Llama-Server_RTX5080.bat`)
* **Uso:** Para cualquier otro modelo que NO posea cabezas MTP (modelos Dense o cuantizaciones estándar).
* **Ruta de Ejecución:** `build\bin\llama-server.exe`
* **Comando Literal Exacto:**
  ```cmd
  build\bin\llama-server.exe -m "C:\Users\<TU_USUARIO>\.lmstudio\models\unsloth\Qwen3.6-35B-A3B-MTP-IQ3_XXS-GGUF\Qwen3.6-35B-A3B-UD-IQ3_XXS.gguf" --flash-attn on -ngl 99 -c 65536 -np 1 -ctk q4_0 -ctv q4_0 --spec-type ngram-map-k --host 127.0.0.1 --port 5050 --jinja
  ```
...
### ⚙️ Configuración del Agente Hermes CLI (`C:\data\hermes\hermes-tui\data\config.yaml`)
