...
:: Leer resultados
for /f "usebackq tokens=1-19 delims=|" %%A in (`powershell -NoProfile -ExecutionPolicy Bypass -File "%LLAMA_DIR%\launch_state_query.ps1" -Query config`) do (
    set "MODEL_PATH=%%A"
    set "VAL_CTX=%%B"
    set "VAL_NGL=%%C"
    set "VAL_NP=%%D"
...
echo [%date% %time%] paso4 config leida: MODEL=!MODEL_PATH! SPEC=dflash=!VAL_DFLASH! mtp=!VAL_MTP! >> "%DBGLOG%"
...
if "!VAL_DFLASH!"=="1" goto SPEC_DFLASH
if "!VAL_MTP!"=="1" goto SPEC_MTP
goto SPEC_DONE
:SPEC_DFLASH
if not exist "!VAL_DRAFTPATH!" goto SPEC_DFLASH_NOFILE
...
goto SPEC_DONE
:SPEC_DFLASH_NOFILE
echo  [!] DFlash activado pero el drafter no existe: !VAL_DRAFTPATH!
echo  [!] Falling back to ngram-map-k. Check the model folder (dflash-kquant.gguf).
...
:SPEC_DONE
echo [%date% %time%] paso13 SPEC resuelto: !SPEC_STATUS! >> "%DBGLOG%"
...
:: Optimización automática de relojes GPU (falla silenciosamente sin admin; opcional)
nvidia-smi -pm 1 >nul 2>&1
nvidia-smi -lgc 3090 >nul 2>&1
nvidia-smi -lmc 14001 >nul 2>&1

echo [%date% %time%] lanzando server: "!BIN_EXE!" -m "!MODEL_PATH!" !SPEC_ARGS! >> "%DBGLOG%"
echo [%date% %time%] BIN_EXE=!BIN_EXE! >> "%DBGLOG%"
if exist "!BIN_EXE!" echo [%date% %time%] BIN_EXE existe SI >> "%DBGLOG%"
if not exist "!BIN_EXE!" echo [%date% %time%] BIN_EXE NO EXISTE >> "%DBGLOG%"
if exist "!MODEL_PATH!" echo [%date% %time%] MODEL existe SI >> "%DBGLOG%"
if not exist "!MODEL_PATH!" echo [%date% %time%] MODEL NO EXISTE >> "%DBGLOG%"
if "!VAL_DFLASH!"=="1" if exist "!VAL_DRAFTPATH!" echo [%date% %time%] DRAFT existe SI >> "%DBGLOG%"
if "!VAL_DFLASH!"=="1" if not exist "!VAL_DRAFTPATH!" echo [%date% %time%] DRAFT NO EXISTE >> "%DBGLOG%"

set "SERVER_LOG=%LLAMA_DIR%\server_output.log"
echo [%date% %time%] server log: %SERVER_LOG% >> "%DBGLOG%"
echo.
echo  ─── SERVER STARTING (live output) ────────────────────────
echo.
echo  ⏳ Loading model... DO NOT CLOSE THIS WINDOW.
echo     After the startup log there are ~30s of SILENCE (loading 12GB to VRAM).
echo     Server is ready when you see "listening on http://127.0.0.1:5050"
echo.
"!BIN_EXE!" -m "!MODEL_PATH!" --flash-attn on -fit off -ngl !VAL_NGL! -c !VAL_CTX! -np !VAL_NP! !KV_ARGS! !B_ARGS! !MCP_ARGS! !SPEC_ARGS! !SAMPLING_ARGS! --host !L_HOST! --port !REAL_PORT! --jinja
