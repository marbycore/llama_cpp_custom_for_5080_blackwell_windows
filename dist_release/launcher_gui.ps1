...
# Al seleccionar un modelo en la grilla, auto-setear spec segun metadata (Muse -> DFlash, MTP -> MTP)
$dgv.Add_SelectionChanged({
        if ($dgv.SelectedRows.Count -gt 0) {
            $selPath = $dgv.SelectedRows[0].Cells[4].Value
            if ($selPath) {
                $isMuse = Get-MuseCapable $selPath
                $isMtp = Get-MtpCapable $selPath
                ...
                if ($isMuse) {
                    ...
                    if ($dr) {
                        ...
                        $lblDflashPath.Text = "Drafter: NO ENCONTRADO - revisar carpeta del modelo"
                        $lblDflashPath.ForeColor = [Drawing.Color]::OrangeRed
                        $chkDflash.Checked = $false
                    }
                } else {
                    $lblDflashPath.Text = "Drafter: no aplica (modelo no-Muse)"
                    $lblDflashPath.ForeColor = [Drawing.Color]::DarkGray
                }
            }
        }
