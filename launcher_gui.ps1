...
if (-not ([System.Management.Automation.PSTypeName]'GgufDetector').Type) {
...
public class GgufDetector {
    public static string DetectMtp(string filePath) {
        if (string.IsNullOrEmpty(filePath) || !File.Exists(filePath)) return "NO";

        string dir = Path.GetDirectoryName(filePath);
        bool hasExt = false;
        try {
            var files = Directory.GetFiles(dir, "mtp-*.gguf");
            foreach (var f in files) {
                if (!f.Equals(filePath, StringComparison.OrdinalIgnoreCase)) {
                    hasExt = true;
                    break;
                }
            }
        ...
        try {
            ...
            using (var br = new BinaryReader(fs)) {
                ...
                if (Encoding.ASCII.GetString(magic) == "GGUF") {
                    ...
                    ulong nTensors = br.ReadUInt64();
                    ulong nKv = br.ReadUInt64();

                    for (ulong i = 0; i < nKv; i++) {
                        ulong kLen = br.ReadUInt64();
                        if (kLen > 65536) break;
                        fs.Seek((long)kLen, SeekOrigin.Current);
                        uint vt = br.ReadUInt32();
                        SkipKvVal(br, fs, vt);
                    }

                    for (ulong i = 0; i < nTensors; i++) {
                        ulong tLen = br.ReadUInt64();
                        if (tLen > 65536) break;
                        byte[] nameBytes = br.ReadBytes((int)tLen);
                        string tName = Encoding.UTF8.GetString(nameBytes);
                        uint ndims = br.ReadUInt32();
                        fs.Seek(ndims * 8 + 12, SeekOrigin.Current);

                        if (tName.Contains("nextn")) {
                            hasInt = true;
                            break;
                        }
                    }
        ...
        if (hasInt && hasExt) return "Ambos";
        if (hasInt) return "Interno";
        if (hasExt) return "Externo";
        return "NO";
    }

    private static void SkipKvVal(BinaryReader br, Stream fs, uint vt) {
        switch (vt) {
            case 0: case 1: case 7: fs.Seek(1, SeekOrigin.Current); break;
            case 2: case 3: fs.Seek(2, SeekOrigin.Current); break;
            case 4: case 5: case 6: fs.Seek(4, SeekOrigin.Current); break;
            case 10: case 11: case 12: fs.Seek(8, SeekOrigin.Current); break;
            case 8:
                ulong sLen = br.ReadUInt64();
                if (sLen <= 65536) fs.Seek((long)sLen, SeekOrigin.Current);
                break;
            case 9:
                uint itemType = br.ReadUInt32();
                ulong arrLen = br.ReadUInt64();
                for (ulong j = 0; j < arrLen; j++) {
                    SkipKvVal(br, fs, itemType);
                }
                break;
        }
    }
...
# ── Detección de MTP por inspección binaria ultra-rápida en C# (<15ms) ──
function Get-MtpKind {
    param([string]$fullName)
    return [GgufDetector]::DetectMtp($fullName)
}

function Get-MtpCapable {
    param([string]$fullName)
    $kind = Get-MtpKind $fullName
    return ($kind -ne 'NO')
}

# ── Detección de Muse Glimmer (necesita DFlash drafter, no MTP) ──
function Get-MuseCapable {
    param([string]$fullName)
    $fileName = Split-Path $fullName -Leaf
    $dirName = Split-Path (Split-Path $fullName -Parent) -Leaf
    return (($fileName -match "muse") -or ($dirName -match "muse"))
}

# ── Buscar el drafter DFlash junto al modelo (dflash*.gguf o *assistant*.gguf) ──
function Get-DflashPath {
    param([string]$modelFullName)
    $modelDir = Split-Path $modelFullName -Parent
    $candidates = @(Get-ChildItem -Path $modelDir -Filter "*.gguf" -ErrorAction SilentlyContinue | Where-Object { $_.Name -match "dflash|draft|assistant" -and $_.FullName -ne $modelFullName })
    if ($candidates.Count -gt 0) { return $candidates[0].FullName }
    $parentDir = Split-Path $modelDir -Parent
    $candidates2 = @(Get-ChildItem -Path $parentDir -Filter "*dflash*.gguf" -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.FullName -ne $modelFullName })
    if ($candidates2.Count -gt 0) { return $candidates2[0].FullName }
    return ""
}

# ── Buscar el proyector de visión MMProj junto al modelo (mmproj*.gguf) ──
function Get-MmprojPath {
    param([string]$modelFullName)
    if (-not $modelFullName -or -not (Test-Path $modelFullName)) { return "" }
    $modelDir = Split-Path $modelFullName -Parent
    # Filtro estricto: nombre debe EMPEZAR con mmproj (evita falsos positivos)
    $candidates = @(Get-ChildItem -Path $modelDir -Filter "mmproj*.gguf" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -ne $modelFullName })
    if ($candidates.Count -gt 0) { return $candidates[0].FullName }
    $parentDir = Split-Path $modelDir -Parent
    $candidates2 = @(Get-ChildItem -Path $parentDir -Filter "mmproj*.gguf" -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.FullName -ne $modelFullName })
    if ($candidates2.Count -gt 0) { return $candidates2[0].FullName }
    return ""
}

# ── Spec del modelo: dflash / mtp / ngram ──
function Get-SpecKind {
    param([string]$fullName)
    if (Get-MuseCapable $fullName) { return "dflash" }
    if (Get-MtpCapable $fullName) { return "mtp" }
    return "ngram"
}
...
$dgv.ColumnCount = 5; $dgv.Columns[0].Name = "Modelo"; $dgv.Columns[0].Width = 330; $dgv.Columns[1].Name = "GB"; $dgv.Columns[1].Width = 60; $dgv.Columns[2].Name = "Origen"; $dgv.Columns[2].Width = 230; $dgv.Columns[3].Name = "MTP"; $dgv.Columns[3].Width = 80; $dgv.Columns[4].Visible = $false
...
# ── DFlash toggle (Muse Glimmer: draft-dflash n=15 + drafter propio) ──
$chkDflash = New-Object Windows.Forms.CheckBox
$chkDflash.Text = "DFlash (Muse Glimmer)"; $chkDflash.Checked = $false
$chkDflash.Location = New-Object Drawing.Point(15, 235); $chkDflash.AutoSize = $true
$chkDflash.ForeColor = [Drawing.Color]::Cyan
$chkDflash.Font = New-Object Drawing.Font("Segoe UI", 9, [Drawing.FontStyle]::Bold); $pnlInner.Controls.Add($chkDflash)

$lblDflashPath = New-Object Windows.Forms.Label
$lblDflashPath.Text = "Drafter: no detectado"
$lblDflashPath.ForeColor = [Drawing.Color]::DarkGray
$lblDflashPath.Location = New-Object Drawing.Point(200, 235); $lblDflashPath.Size = New-Object Drawing.Size(520, 20); $lblDflashPath.Font = New-Object Drawing.Font("Segoe UI", 8)
$pnlInner.Controls.Add($lblDflashPath)

# ── Visión / MMProj toggle (Analizador de Imagen) ──
$chkMmproj = New-Object Windows.Forms.CheckBox
$chkMmproj.Text = "Analizador de Imagen (MMProj / Visión)"
$chkMmproj.Checked = $false
$chkMmproj.Enabled = $false
$chkMmproj.Location = New-Object Drawing.Point(15, 265); $chkMmproj.AutoSize = $true
$chkMmproj.ForeColor = [Drawing.Color]::Magenta
$chkMmproj.Font = New-Object Drawing.Font("Segoe UI", 9, [Drawing.FontStyle]::Bold); $pnlInner.Controls.Add($chkMmproj)

$lblMmprojPath = New-Object Windows.Forms.Label
$lblMmprojPath.Text = "MMProj: No disponible en la carpeta"
$lblMmprojPath.ForeColor = [Drawing.Color]::DarkGray
$lblMmprojPath.Location = New-Object Drawing.Point(310, 265); $lblMmprojPath.Size = New-Object Drawing.Size(560, 20); $lblMmprojPath.Font = New-Object Drawing.Font("Segoe UI", 8)
$pnlInner.Controls.Add($lblMmprojPath)

# ── Persistencia de Configuración Individual por Modelo ──
$PROFILE_FILE = Join-Path (Split-Path $LAUNCH_STATE) "model_profiles.json"
$script:MODEL_PROFILES = @{}
if (Test-Path $PROFILE_FILE) {
    try {
        $jsonP = Get-Content $PROFILE_FILE -Raw | ConvertFrom-Json
        foreach ($prop in $jsonP.PSObject.Properties) {
            $script:MODEL_PROFILES[$prop.Name] = $prop.Value
        }
    }
    catch {}
}
$script:LAST_MODEL_PATH = ""

function Save-CurrentModelProfile($mPath) {
    if (-not $mPath) { return }
    $inv = [System.Globalization.CultureInfo]::InvariantCulture
    $prof = @{
        ctx      = $cCtx.SelectedIndex
        ngl      = $cGpu.SelectedIndex
        np       = $cPar.SelectedIndex
        ub       = $cBat.SelectedIndex
        kv       = $cKv.SelectedIndex
        kvD      = $cKvD.SelectedIndex
        mtp      = $chkMtp.Checked
        dflash   = $chkDflash.Checked
        mmproj   = $chkMmproj.Checked
        temp     = (($sTemp.Track.Value * $sTemp.Step).ToString($sTemp.Format, $inv)).Replace(',', '.')
        topP     = (($sTopP.Track.Value * $sTopP.Step).ToString($sTopP.Format, $inv)).Replace(',', '.')
        topK     = (($sTopK.Track.Value * $sTopK.Step).ToString($sTopK.Format, $inv)).Replace(',', '.')
        minP     = (($sMinP.Track.Value * $sMinP.Step).ToString($sMinP.Format, $inv)).Replace(',', '.')
        presence = (($sPresence.Track.Value * $sPresence.Step).ToString($sPresence.Format, $inv)).Replace(',', '.')
        repeat   = (($sRepeat.Track.Value * $sRepeat.Step).ToString($sRepeat.Format, $inv)).Replace(',', '.')
    }
    $script:MODEL_PROFILES[$mPath] = $prof
    try {
        $script:MODEL_PROFILES | ConvertTo-Json -Depth 5 | Out-File $PROFILE_FILE -Force -Encoding utf8
    }
    catch {}
}

function Load-ModelProfile($mPath) {
    if (-not $mPath) { return }
    if ($script:MODEL_PROFILES.ContainsKey($mPath)) {
        $p = $script:MODEL_PROFILES[$mPath]
        if ($null -ne $p.ctx) { $cCtx.SelectedIndex = [int]$p.ctx }
        if ($null -ne $p.ngl) { $cGpu.SelectedIndex = [int]$p.ngl }
        if ($null -ne $p.np) { $cPar.SelectedIndex = [int]$p.np }
        if ($null -ne $p.ub) { $cBat.SelectedIndex = [int]$p.ub }
        if ($null -ne $p.kv) { $cKv.SelectedIndex = [int]$p.kv }
        if ($null -ne $p.kvD) { $cKvD.SelectedIndex = [int]$p.kvD }
        if ($null -ne $p.mtp) { $chkMtp.Checked = [bool]$p.mtp }
        if ($null -ne $p.dflash) { $chkDflash.Checked = [bool]$p.dflash }
        if ($null -ne $p.mmproj -and $chkMmproj.Enabled) { $chkMmproj.Checked = [bool]$p.mmproj } else { $chkMmproj.Checked = $false }
        if ($null -ne $p.temp -and $sTemp) {
            $val = [double]$p.temp
            $sTemp.Track.Value = [int]($val / $sTemp.Step); $sTemp.Lbl.Text = $val.ToString($sTemp.Format, [System.Globalization.CultureInfo]::InvariantCulture)
        }
        if ($null -ne $p.topP -and $sTopP) {
            $val = [double]$p.topP
            $sTopP.Track.Value = [int]($val / $sTopP.Step); $sTopP.Lbl.Text = $val.ToString($sTopP.Format, [System.Globalization.CultureInfo]::InvariantCulture)
        }
        if ($null -ne $p.topK -and $sTopK) {
            $val = [double]$p.topK
            $sTopK.Track.Value = [int]($val / $sTopK.Step); $sTopK.Lbl.Text = $val.ToString($sTopK.Format, [System.Globalization.CultureInfo]::InvariantCulture)
        }
    }
    else {
        # Configuración por defecto inteligente: 128K contexto siempre, q4_0 KV, MTP activo, MMProj desactivado
        $cCtx.SelectedIndex = 2 # 131072 (128K)
        $cKv.SelectedIndex = 0  # q4_0
        $isMuse = Get-MuseCapable $mPath
        $isMtp = Get-MtpCapable $mPath
        $chkMtp.Checked = ($isMtp -and -not $isMuse)
        $chkDflash.Checked = $isMuse
        $chkMmproj.Checked = $false
        if ($isMuse -and $sTemp) {
            $sTemp.Track.Value = [int](1.0 / $sTemp.Step); $sTemp.Lbl.Text = "1.0"
            $sTopK.Track.Value = [int](64 / $sTopK.Step); $sTopK.Lbl.Text = "64"
            $sTopP.Track.Value = [int](0.95 / $sTopP.Step); $sTopP.Lbl.Text = "0.95"
        }
        elseif ($sTemp) {
            $sTemp.Track.Value = [int](0.4 / $sTemp.Step); $sTemp.Lbl.Text = "0.4"
            $sTopK.Track.Value = [int](20 / $sTopK.Step); $sTopK.Lbl.Text = "20"
            $sTopP.Track.Value = [int](0.95 / $sTopP.Step); $sTopP.Lbl.Text = "0.95"
        }
    }
}

# Al seleccionar un modelo en la grilla, guardar el perfil anterior y cargar el nuevo
$dgv.Add_SelectionChanged({
        if ($dgv.SelectedRows.Count -gt 0) {
            $selPath = $dgv.SelectedRows[0].Cells[4].Value
            if ($selPath -and $selPath -ne $script:LAST_MODEL_PATH) {
                if ($script:LAST_MODEL_PATH) { Save-CurrentModelProfile $script:LAST_MODEL_PATH }
                $script:LAST_MODEL_PATH = $selPath
                $dr = Get-DflashPath $selPath
                if ($dr) {
                    $lblDflashPath.Text = "Drafter: $(Split-Path $dr -Leaf)"
                    $lblDflashPath.ForeColor = [Drawing.Color]::Cyan
                }
                else {
                    $lblDflashPath.Text = "Drafter: no detectado o integrado en el modelo"
                    $lblDflashPath.ForeColor = [Drawing.Color]::DarkGray
                }
                $mm = Get-MmprojPath $selPath
                if ($mm) {
                    $chkMmproj.Enabled = $true
                    $lblMmprojPath.Text = "MMProj detectado: $(Split-Path $mm -Leaf)"
                    $lblMmprojPath.ForeColor = [Drawing.Color]::LimeGreen
                }
                else {
                    $chkMmproj.Enabled = $false
                    $chkMmproj.Checked = $false
                ...
                Load-ModelProfile $selPath
            }
        }
    })

# Sincronizar estado inicial de la fila 0
if ($dgv.SelectedRows.Count -gt 0) {
    $selPath = $dgv.SelectedRows[0].Cells[4].Value
    if ($selPath) {
        $script:LAST_MODEL_PATH = $selPath
...
$chkLan.Location = New-Object Drawing.Point(15, 295); $chkLan.AutoSize = $true; $chkLan.ForeColor = [Drawing.Color]::DeepSkyBlue
...
$chkTavily.Location = New-Object Drawing.Point(350, 295); $chkTavily.AutoSize = $true
...
$lblTavilyKey.Location = New-Object Drawing.Point(350, 312); $lblTavilyKey.AutoSize = $true
...
$lblSampling.Text = "SAMPLING (defaults = config ultra testeada Qwen; ajustar por modelo si hace falta)"; $lblSampling.Location = New-Object Drawing.Point(15, 340); $lblSampling.AutoSize = $true; $lblSampling.ForeColor = [Drawing.Color]::LimeGreen; $lblSampling.Font = New-Object Drawing.Font("Segoe UI", 9, [Drawing.FontStyle]::Bold)
...
function New-Slider {
    ...
    return @{ Track = $tb; Step = $step; Format = $format; Lbl = $lblVal }
}

$sTemp = New-Slider "Temperature" 15 370 0.0 2.0 0.4 0.1 "0.0"
$sTopP = New-Slider "Top P" 15 410 0.5 1.0 0.95 0.01 "0.00"
$sTopK = New-Slider "Top K" 465 370 1 200 20 1 "0"
$sMinP = New-Slider "Min P" 465 410 0.0 1.0 0.0 0.01 "0.00"
$sPresence = New-Slider "Presence Penalty" 15 450 0.0 2.0 0.0 0.1 "0.0"
$sRepeat = New-Slider "Repeat Penalty" 465 450 1.0 2.0 1.0 0.05 "0.00"
...
$btn.Add_Click({
        ...
        if ($modelPath) { Save-CurrentModelProfile $modelPath }
        ...
        $dflashVal = if ($chkDflash.Checked) { "1" }else { "0" }
        $draftPath = if ($chkDflash.Checked) { Get-DflashPath $modelPath } else { "" }
        $mmprojVal = if ($chkMmproj.Checked -and $chkMmproj.Enabled) { "1" } else { "0" }
        $mmprojPath = if ($mmprojVal -eq "1") { Get-MmprojPath $modelPath } else { "" }
        $lan = if ($chkLan.Checked) { "1" }else { "0" }
        $tavily = if ($chkTavily.Checked) { "1" }else { "0" }
        $inv = [System.Globalization.CultureInfo]::InvariantCulture
        $tempVal = (($sTemp.Track.Value * $sTemp.Step).ToString($sTemp.Format, $inv)).Replace(',', '.')
        $topPVal = (($sTopP.Track.Value * $sTopP.Step).ToString($sTopP.Format, $inv)).Replace(',', '.')
        $topKVal = (($sTopK.Track.Value * $sTopK.Step).ToString($sTopK.Format, $inv)).Replace(',', '.')
        $minPVal = (($sMinP.Track.Value * $sMinP.Step).ToString($sMinP.Format, $inv)).Replace(',', '.')
        $presenceVal = (($sPresence.Track.Value * $sPresence.Step).ToString($sPresence.Format, $inv)).Replace(',', '.')
        $repeatVal = (($sRepeat.Track.Value * $sRepeat.Step).ToString($sRepeat.Format, $inv)).Replace(',', '.')
        $state = @{
            mode    = "gui"
            config  = @{
                modelPath  = $modelPath
...
if ($Headless) {
    if ($Status) {
        if (Test-Path $LAUNCH_STATE) {
            ...
            "MODE: $($s.mode)"
            "CONFIG: $($s.config.modelPath) | ctx=$($s.config.ctx) ngl=$($s.config.ngl) np=$($s.config.np) ub=$($s.config.ub) kv=$($s.config.kv) kvd=$($s.config.kvD) mtp=$($s.config.mtp) dflash=$($s.config.dflash) draft=$($s.config.draftPath) temp=$($s.config.temp) topP=$($s.config.topP) topK=$($s.config.topK) minP=$($s.config.minP) presence=$($s.config.presenceP) repeat=$($s.config.repeatP) lan=$($s.config.lan) tavily=$($s.config.tavily)"
            "UPDATED: $($s.updated)"
        }
        else {
    ...
    $isMuseH = Get-MuseCapable $ModelPath
    $dflashVal = $Dflash
    if ($dflashVal -eq "auto") { $dflashVal = if ($isMuseH) { "1" } else { "0" } }
    $draftPathH = $DraftPath
    if ($dflashVal -eq "1" -and -not $draftPathH) { $draftPathH = Get-DflashPath $ModelPath }
    if ($isMuseH) { $mtpVal = "0" }
    # Sampling recomendado por Meta para Muse Glimmer: temp 1.0 / top_p 0.95 / top_k 64
    # (solo si el usuario no los seteo explicitamente)
    $tempH = $Temp; $topPH = $TopP; $topKH = $TopK
    if ($isMuseH -and $Temp -eq "0.4" -and $TopK -eq "20") {
        $tempH = "1.0"; $topPH = "0.95"; $topKH = "64"
    }
...
if ($Test) {
    ...
    $state = @{
        ...
        config  = @{
            ...
            dflash     = if (Get-MuseCapable $testPath) { "1" } else { "0" }
            draftPath  = if (Get-MuseCapable $testPath) { Get-DflashPath $testPath } else { "" }
            mmproj     = "0"
            mmprojPath = ""
            temp       = "0.4"
            topP       = "0.95"
